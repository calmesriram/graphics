...
bank_d1 : function(){
    ...
    Bank.deployed().then(function(instance) {
     ...
     // document.getElementById('borrow').value = val[7];
     // document.getElementById('lend').value= val[8];
    });
  },
*/
  ...
  $("#register-bank").click(function(event) {
    ...
    var deposit_inter = document.getElementById("deposit-interest").value*100;
    var bank_name = document.getElementById("bank-name").value;

    $("#status").html("Initiating transaction... (please wait)");

    Bank.deployed().then(function(instance) {
      return instance.register(bank_name, deposit_inter, loan_interest, interest, {from: account, gas: 6000000});
    }).then(function(val) {
      if(account== val)
      {
         alert('alredy exitst');
      ...
      return instance.show_registers();
       ...
       $.each(val,function(err,data){
        bank.bank_d1(data).then(function(result){
          ...
          $("#body_bank").append('<tr><td>'+data+'</td><td>'+result[0]+'</td><td>'+result[1].toNumber()+'</td><td>'+(result[3]/100)+" %"+'</td><td>'+(result[4]/100)+" %"+'</td><td>'+(result[5]/100)+" %"+'</td></tr>');
        })
       })
